...
def power():
    ...
    if kind=="bomb":
        ...
        me["powers"][kind]-=1; me["power_used_this_turn"]+=1
        b=r["players"][target]["board"]; count=0
        ...
        if h:
            for xx in range(10):
                if isinstance(b[y][xx],int) and b[y][xx]>0: count+=1; intel(r,p,target,xx,y)
            msg=f"Sonar Reihe {y+1}: {count} Schiffsfelder markiert."
        else:
            for yy in range(10):
                if isinstance(b[yy][x],int) and b[yy][x]>0: count+=1; intel(r,p,target,x,yy)
...
async function post(url,data){const r=await fetch(url,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(data)});const j=await r.json();if(!r.ok){showOverlay(j.error||"Fehler");return null}if(j.privateMessage)showOverlay(j.privateMessage);return j}
